#!/usr/bin/env python3
# nobitex_ohlc_scan.py
# Fetch OHLC from Nobitex UDF/history and compute simple TA + signal
# Usage:
# export NOBITEX_TOKEN="..." 
# python nobitex_ohlc_scan.py --symbol ZECUSDT --resolution 60 --from-hrs 24 --to-now
...
API_BASE = "https://api.nobitex.ir/market/udf/history"
...
def fetch_ohlc(symbol, resolution, from_unix, to_unix, headers=HEADERS, timeout=15):
...
def ohlc_json_to_df(j):
    # expects keys t,o,c,l,h,v as lists
    if not isinstance(j, dict):
        raise ValueError("Unexpected response shape")
    if not all(k in j for k in ("t","o","c","h","l","v")):
        raise ValueError("Missing OHLC keys in response")
    df = pd.DataFrame({
        "t": j["t"],
        "Open": j["o"],
        "Close": j["c"],
...
def main():
    ...
    p.add_argument("--resolution", default="60", help="candle resolution (60 for 1h, D for daily, etc.)")
    p.add_argument("--from-hrs", type=float, default=72, help="how many hours back to fetch (relative to now)")
    p.add_argument("--to-now", action="store_true", default=True)
    p.add_argument("--from-unix", type=int, help="explicit from unix seconds")
    p.add_argument("--to-unix", type=int, help="explicit to unix seconds")
    args = p.parse_args()

    if args.from_unix:
        from_unix = args.from_unix
    else:
        from_dt = datetime.utcnow() - timedelta(hours=args.from_hrs)
        from_unix = unix_ts(from_dt)
    if args.to_unix:
    ...
    try:
        ...
        print("Failed to parse OHLC response:", e)
