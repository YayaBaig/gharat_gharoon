#!/usr/bin/env python3
# nobitex_signals_nominal_fees_with_ohlc.py
# نسخه نهایی: محاسبه با کارمزد و لغزش، رسم کندل 1h واقعی داخل HTML، و خروجی CSV خلاصه فارسی
...
OUT_HTML = "نوبیتکس_سیگنال‌ها_با_کندل_1h.html"
OUT_SUMMARY = "خلاصه_سیگنال‌ها_نوبیتکس_ساده.csv"
...
def build_html_with_ohlc(df_signals, out_html, headers=None):
    ...
    for _, row in df_signals.iterrows():
        sym = row['symbol']
        sig = row['signal_obj']
        # try to fetch OHLC 1h for lookback (default 72h)
        ...
        if df_ohlc is not None and len(df_ohlc) > 1:
            ...
            # zoom last 48 candles if available
            if len(df_ohlc) > 48:
                x0 = df_ohlc["Date"].iloc[-48]
                fig.update_xaxes(range=[x0, df_ohlc["Date"].iloc[-1]], row=r, col=1)
        else:
            # fallback: draw flat line at last_price
            price = float(row['last_price'])
            fig.add_trace(go.Scatter(x=[0,1], y=[price,price], mode='lines', line=dict(color='lightgray'), name=f"{sym} price"), row=r, col=1)

        # markers for entry/stop/target (x positions chosen for visibility)
        last_x = df_ohlc["Date"].iloc[-1] if df_ohlc is not None and len(df_ohlc)>0 else 1
        # use small offsets in x for placing markers
        fig.add_trace(go.Scatter(x=[last_x - pd.Timedelta(minutes=60*2)], y=[sig['entry']], mode='markers+text', marker=dict(color='blue', size=8), text=[f"ورود {sig['entry']:.6g}"], textposition="top center", showlegend=False), row=r, col=1)
        fig.add_trace(go.Scatter(x=[last_x - pd.Timedelta(minutes=60)], y=[sig['stop']], mode='markers+text', marker=dict(color='red', size=8), text=[f"استاپ {sig['stop']:.6g}"], textposition="bottom center", showlegend=False), row=r, col=1)
        fig.add_trace(go.Scatter(x=[last_x], y=[sig['target']], mode='markers+text', marker=dict(color='green', size=8), text=[f"تارگت {sig['target']:.6g}"], textposition="top center", showlegend=False), row=r, col=1)

        # annotation with stats (فارسی)
        stats_text = (f"واحدها: {row['واحدها']:.6f}<br>"
                      f"سود خالص پس از کارمزد و لغزش: {row['سود خالص پس از کارمزد و لغزش (USD)']:.2f} USD<br>"
                      f"پیپ(تقریبی): {row['پیپ(تقریبی)']:.6g}<br>"
                      f"سود پیپ: {row['سود پیپ']:.1f}<br>"
                      f"قیمت اجرایی ورود: {row['قیمت اجرایی ورود']:.6f}")
        if r == 1:
    ...
    fig.update_layout(height=380*n, title_text="سیگنال‌ها و کندل‌های 1h (نوبیتکس)", showlegend=False, margin=dict(l=40, r=260, t=80, b=40))
...
def main():
    ...
    # خروجی CSV ساده با ستون‌های درخواستی
    out_simple = pd.DataFrame({
        "نماد": df2["symbol"],
        "قیمت_آخر (USD)": df2["last_price"],
        "سود خالص پس از کارمزد و لغزش (USD)": df2["سود خالص پس از کارمزد و لغزش (USD)"],
        "پیپ(تقریبی)": df2["پیپ(تقریبی)"],
        "سود پیپ": df2["سود پیپ"],
        "قیمت اجرایی ورود": df2["قیمت اجرایی ورود"]
    })
    out_simple.to_csv(OUT_SUMMARY, index=False, encoding='utf-8-sig')
    print("خلاصه ساده ذخیره شد:", OUT_SUMMARY)

    # HTML با کندل 1h واقعی برای هر سیگنال
    html_path = build_html_with_ohlc(df2.reset_index(drop=True), OUT_HTML, headers=HEADERS)
    print("نمودار تعاملی ساخته شد:", html_path)

    # توضیح ساده برای درک نمودار (چاپ در کنسول)
    help_text = """
...
- نقاط آبی/قرمز/سبز روی نمودار به ترتیب نشان‌دهنده قیمت اجرایی ورود، استاپ و تارگت هستند.
